#!/bin/bash
# Transaction Manager 服务管理脚本
# 用于 transaction_manager 角色，使用 Qwen2.5-7B 模型
...
SERVICE_NAME="transaction_manager"
...
start_service() {
    ...
    echo "启动 Transaction Manager 服务 (Qwen2.5-7B)..."
...
# 停止服务
stop_service() {
    echo "停止 Transaction Manager 服务..."
...
# 重启服务
restart_service() {
    echo "重启 Transaction Manager 服务..."
    stop_service
    echo ""
    sleep 2
    start_service
...
case "$1" in
    ...
    check)
        if check_port; then
            echo "✓ Transaction Manager 服务正常运行"
            exit 0
        else
            echo "✗ Transaction Manager 服务不可用"
            exit 1
        fi
        ;;
    *)
