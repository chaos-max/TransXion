#!/bin/bash
# Merchant Generator 服务管理脚本
# 用于 merchant_generator 角色，使用 Qwen3-0.6B 模型

# 配置（请根据实际环境修改）
MODEL_PATH="${MODEL_PATH:-/path/to/your/Qwen3-0.6B}"
PORT="${PORT:-8002}"
LOG_DIR="${LOG_DIR:-logs}"
GPU_MEMORY_UTILIZATION="${GPU_MEMORY_UTILIZATION:-0.3}"
GPU_DEVICES="${GPU_DEVICES:-2}"
TENSOR_PARALLEL_SIZE="${TENSOR_PARALLEL_SIZE:-}"
SERVICE_NAME="merchant_generator"
...
start_service() {
    ...
    echo "启动 Merchant Generator 服务 (Qwen3-0.6B)..."
...
case "$1" in
    ...
    check)
        if check_port; then
            echo "✓ Merchant Generator 服务正常运行"
            exit 0
        else
            echo "✗ Merchant Generator 服务不可用"
            exit 1
        fi
        ;;
    *)
